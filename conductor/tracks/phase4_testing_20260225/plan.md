...
## Phase 1: Pre-request Scripts ✅ IN_PROGRESS
...
## Phase 2: Response Assertions (Tests) ✅ IN_PROGRESS
...
## Phase 3: Collection Runner ✅ PENDING
- [ ] Task: 创建 "Collection Runner" 独立 UI 窗口，支持集合选择
- [ ] Task: 实现批量运行逻辑，支持 `Pre-request` -> `Request` -> `Tests` 完整生命周期
- [ ] Task: 实现运行后的统计报告界面 (实时更新进度和通过率)
